...
/**
 * Réalise la validation d'un EObject issu de Test.
 ...
 * @version 0.1
 */
public class TestValidator extends TestSwitch<Boolean> {
	...
	public ValidationResult validate(org.eclipse.emf.ecore.resource.Resource resource) {
		...
		/**
		 * Méthode appelée lorsque l'objet visité est un Entreprise.
		 ...
		 */
		@Override
		public Boolean caseEntreprise(test.Entreprise object) {

			if(!eObject.contains(object)) {
				eObject.add(object);
				this.ocl.check(object);
	
				for (Employe pe : object.getEmploie()) {
		...
		/**
		 * Méthode appelée lorsque l'objet visité est un Employe.
		 * Cet méthode amorce aussi la visite des éléments enfants.
		 * Si l'EOject n'était pas déjà visité on le viste sinon on poursuit la visite.
		 * @param object élément visité
		 * @return résultat de validation (null ici, ce qui permet de poursuivre la visite
		 ...
		 */
		@Override
		public Boolean caseEmploye(test.Employe object) {

			if(!eObject.contains(object)) {
				eObject.add(object);
				this.ocl.check(object);
	
				for (Employe pe : object.getManage()) {
					if(!eObject.contains(pe)) {
						this.doSwitch(pe);
					}
				}
			
				if(!eObject.contains(object.getManager())) {
					this.doSwitch(object.getManager());
				}
